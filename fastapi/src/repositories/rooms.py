...
from src.repositories.mappers.rooms_mapper import RoomsMapper, facilities_to_schema
...
class RoomsRepository(BaseRepository[RoomsOrm]):
    ...
    async def edit(self, id: int, **kwargs) -> Optional[SchemaRoom]:
        """
    ...
    async def get_by_hotel_id(self, hotel_id: int) -> List[SchemaRoom]:
        ...
        Returns:
            Список комнат (Pydantic схемы)
        """
        query = self._load_room_with_facilities_query().where(
    ...
    def _load_room_with_facilities_query(self, room_id: Optional[int] = None):
        """
    ...
    async def get_rooms_with_availability(
        ...
        rooms_query = self._load_room_with_facilities_query().where(
            self.model.hotel_id == hotel_id
        )
    ...
    async def get_orm_by_id(self, room_id: int) -> Optional[RoomsOrm]:
        """
    ...
    async def add_facility(self, room_id: int, facility_id: int) -> bool:
        ...
        check_query = select(rooms_facilities).where(
            and_(
                rooms_facilities.c.room_id == room_id,
                rooms_facilities.c.facility_id == facility_id
            )
        )
        ...
        # Создаем связь
        insert_stmt = rooms_facilities.insert().values(
            room_id=room_id,
    ...
    async def get_room_facilities(self, room_id: int) -> List[int]:
        ...
        query = select(rooms_facilities.c.facility_id).where(
            rooms_facilities.c.room_id == room_id
        )
        result = await self.session.execute(query)
    ...
    async def update_room_facilities(self, room_id: int, new_facility_ids: List[int]) -> None:
        ...
        if to_remove:
            delete_stmt = rooms_facilities.delete().where(
                and_(
                    rooms_facilities.c.room_id == room_id,
                    rooms_facilities.c.facility_id.in_(to_remove)
                )
            )
