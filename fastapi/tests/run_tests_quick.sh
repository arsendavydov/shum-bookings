...
# Быстрый скрипт для запуска тестов без нагрузочного тестирования
# Используется для быстрой проверки кода во время разработки

echo "🚀 Запуск быстрых тестов (без нагрузочного тестирования)..."
...
if [ $UNIT_TEST_EXIT_CODE -ne 0 ] || [ $API_TEST_EXIT_CODE -ne 0 ] || [ $INDEX_TEST_EXIT_CODE -ne 0 ]; then
    TEST_EXIT_CODE=1
    echo "❌ Некоторые тесты не прошли. Нагрузочные тесты не запускаются."
else
    TEST_EXIT_CODE=0
    echo "✅ Все быстрые тесты прошли успешно!"
    echo "💡 Для запуска нагрузочных тестов используйте: ./run_load_tests.sh"
fi
